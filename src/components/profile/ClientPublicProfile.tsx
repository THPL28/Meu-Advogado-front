...
export const ClientPublicProfile: React.FC = () => {
  ...
  const clientJobs = jobs.filter(j => j.clientId === selectedClientId || !selectedClientId);
  const sampleJob = clientJobs[0] || jobs[0];
  const clientName = sampleJob?.clientName || 'TechCorp Soluções S.A.';
  const clientAvatar = sampleJob?.clientAvatar || 'https://images.unsplash.com/photo-1549923746-c502d488b3ea?auto=format&fit=crop&q=80&w=256';

  const completedCount = clientJobs.filter(j => j.status === 'COMPLETED').length + 4;
  const inProgressCount = clientJobs.filter(j => j.status === 'IN_PROGRESS').length + 2;
  const totalPostedCount = clientJobs.length + 6;

  // Mock past reviews given by client to lawyers
  const clientReviews = [
    {
      id: 'rev_1',
      ...
      rating: 5,
      projectTitle: 'Adequação LGPD e Revisão de Termos SaaS',
      comment: 'Atendimento excepcional! O Dr. Rodrigo entregou todas as minutas e a política de privacidade antes do prazo estipulado com rigor técnico impecável.',
      date: '15/01/2025'
    },
    {
      id: 'rev_2',
      lawyerName: 'Dra. Patricia Lima',
      lawyerOab: 'OAB/RJ 198.420',
      lawyerAvatar: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&q=80&w=256',
      rating: 4.9,
      projectTitle: 'Auditoria de Passivo Trabalhista Corporativo',
      comment: 'Profissional extremamente técnica e transparente na condução do parecer. Recomendo fortemente para demandas complexas.',
      date: '02/12/2024'
    }
  ];
            ...
            <div className="space-y-1.5 pt-2 sm:pt-4">
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs font-medium text-muted-foreground/90 pt-0.5">
                <span className="flex items-center gap-1">
                  <MapPin className="w-3.5 h-3.5 text-muted-foreground/90" /> São Paulo, SP • Brasil
                </span>
                <span className="flex items-center gap-1">
                  <Calendar className="w-3.5 h-3.5 text-muted-foreground/90" /> Integrante desde Janeiro de 2024
                </span>
                <span className="flex items-center gap-1 text-amber-600 font-bold">
                  <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-400" /> 4.95 (18 avaliações)
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 bg-card border border-border/80 rounded-3xl space-y-4 shadow-xs">
            ...
            <p className="text-xs text-muted-foreground/90 leading-relaxed">
              Empresa líder no setor de tecnologia e software SaaS, focada na inovação de meios de pagamento e conformidade regulatória. Contrata frequentemente pareceres jurídicos, auditorias de passivos e minutas contratuais especializadas.
            </p>
            ...
            <div className="pt-3 border-t border-border/50 space-y-2 text-xs">
              <div className="flex justify-between text-muted-foreground/90">
                ...
                <span className="font-bold text-foreground/90">Tecnologia & SaaS</span>
              </div>
              <div className="flex justify-between text-muted-foreground/90">
                <span className="text-muted-foreground/90 font-semibold">Porte:</span>
                <span className="font-bold text-foreground/90">Médio Porte (100-500 func)</span>
              </div>
              <div className="flex justify-between text-muted-foreground/90">
